...
update_hyprland_colors() {
  ...
  fg_hex=$(grep -oP '(?<=--color11: )#[0-9a-fA-F]+' "$PYWAL_COLORS_CSS") || {
    echo "[Error]::Failed to parse fg color"
    exit 1
  }
  bg_hex=$(grep -oP '(?<=--color0: )#[0-9a-fA-F]+' "$PYWAL_COLORS_CSS") || {
    echo "[Error]::Failed to parse bg color"
    exit 1
  }
...
reload_services() {
  ...
  bash ~/.config/hypr/services/swaync.sh -r || {
    echo "[Error]::Failed to reload swaync"
    exit 1
  }
  ...
  hyprctl reload || {
    echo "[Error]::Failed to reload hyprland"
    exit 1
  }
}
